...
# Breakout universe selection defaults
...
MIN_DAILY_TRADED_VALUE = 200000000.0
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code in (401, 403, 429, 500, 502, 503, 504):
                # NSE occasionally blocks requests until fresh cookies are set.
                session.get("https://www.nseindia.com/", timeout=8)
...
def is_nse_trading_holiday(check_date=None):
...
def _scan_breakout_candidate(stock):
    try:
        ...
        open_range = intraday.iloc[:OPENING_RANGE_MINUTES]
        if len(open_range) < OPENING_RANGE_MINUTES:
            return None
        ...
        last_vol_sma20 = float(intraday["Volume_SMA20"].iloc[-1]) if pd.notna(intraday["Volume_SMA20"].iloc[-1]) else 0.0
        ema20 = float(intraday["EMA20"].iloc[-1])
        recent_closes = intraday["Close"].iloc[-BREAKOUT_CONFIRM_BARS:] if len(intraday) >= BREAKOUT_CONFIRM_BARS else intraday["Close"].iloc[-1:]
        ...
        if len(cons_window) >= CONSOLIDATION_LOOKBACK_BARS + 1:
            ...
            range_pct = ((float(pre_break["High"].max()) - float(pre_break["Low"].min())) / max(last_close, 1e-6)) * 100
...
def get_dynamic_stocks_with_meta(enable_shorting=ENABLE_SHORTING):
    indexes = {
        'NIFTY50': ['RELIANCE.NS', 'TCS.NS', 'INFY.NS', 'HDFCBANK.NS', 'ICICIBANK.NS', 'HINDUNILVR.NS', 'ITC.NS', 'KOTAKBANK.NS', 'LT.NS', 'AXISBANK.NS', 'BAJFINANCE.NS', 'BHARTIARTL.NS', 'CIPLA.NS', 'DRREDDY.NS', 'GRASIM.NS', 'HCLTECH.NS', 'HEROMOTOCO.NS', 'ICICIBANK.NS', 'INDUSINDBK.NS', 'JSWSTEEL.NS'],
        'NIFTY NEXT 50': ['ADANIENT.NS', 'ADANIGREEN.NS', 'ADANIPOWER.NS', 'ADANITRANS.NS', 'AMBUJACEM.NS', 'APOLLOHOSP.NS', 'AUROPHARMA.NS', 'BAJAJHLDNG.NS', 'BANDHANBNK.NS', 'BERGEPAINT.NS', 'BIOCON.NS', 'BOSCHLTD.NS', 'CADILAHC.NS', 'CHOLAFIN.NS', 'COLPAL.NS', 'DABUR.NS', 'DLF.NS', 'DMART.NS', 'GLAND.NS', 'GODREJPROP.NS'],
        'MIDCAP': ['GODREJCP.NS', 'VOLTAS.NS', 'PAGEIND.NS', 'HAVELLS.NS', 'BAJAJ-AUTO.NS', 'MARUTI.NS', 'EICHERMOT.NS', 'M&M.NS', 'GRASIM.NS', 'ULTRACEMCO.NS', 'BAJAJFINSV.NS', 'DIVISLAB.NS', 'SHREECEM.NS', 'TITAN.NS', 'UPL.NS'],
        'SMALLCAP': ['COALINDIA.NS', 'NMDC.NS', 'SAIL.NS', 'BEL.NS', 'NTPC.NS', 'GAIL.NS', 'ONGC.NS', 'POWERGRID.NS', 'IOC.NS', 'BPCL.NS', 'SUNPHARMA.NS', 'TATACONSUM.NS', 'BRITANNIA.NS', 'NESTLEIND.NS', 'ADANIPORTS.NS']
    }
    ...
    if breakout_candidates:
        ...
        labels = ", ".join(f"{item['stock']}[{item['labels']}]" for item in breakout_candidates[:5])
        return selected, "breakout_scan", f"Selected via ORB/consolidation breakout scan. Top patterns: {labels}"

    # Fallback to legacy momentum selector only when breakout scan yields no candidates.
...
def get_dynamic_stocks(enable_shorting=ENABLE_SHORTING):
    stocks, _, _ = get_dynamic_stocks_with_meta(enable_shorting=enable_shorting)
    return stocks


def is_data_fresh(data, max_age_minutes=LIVE_DATA_MAX_CANDLE_AGE_MINUTES):
...
# Function to fetch intraday data (last trading day, 1-minute intervals)
...
def calculate_indicators(data):
    ...
    cumulative_turnover = (typical_price * data['Volume']).cumsum()
    cumulative_volume = data['Volume'].cumsum().replace(0, float('nan'))
    data['VWAP'] = (cumulative_turnover / cumulative_volume).astype(float)

    return data

# Aggressive trading strategy
def generate_signals(data, enable_shorting=ENABLE_SHORTING):
    if data.empty:
        return data
    ...
    bullish_trend = (data['EMA20'] > data['EMA50']) & (data['Close'] > data['VWAP'])
    bearish_trend = (data['EMA20'] < data['EMA50']) & (data['Close'] < data['VWAP'])

    volume_spike = data['Volume'] > (data['Volume_SMA20'] * 1.10)
    adx_ok = data['ADX'] > 18
    atr_ok = data['ATR_pct'].between(0.25, 4.50)
    obv_up = data['OBV'].diff() > 0
    obv_down = data['OBV'].diff() < 0

    # Mild mean-reversion context around Bollinger bands for better entries.
    near_bb_low = data['Close'] <= (data['BB_low'] * 1.01)
    near_bb_high = data['Close'] >= (data['BB_high'] * 0.99)

    # Explicit trend-strength filter helps reduce choppy entries.
    trend_gap_pct = ((data['EMA20'] - data['EMA50']).abs() / data['Close']) * 100
    ...
    # Confluence scores reduce false signals from standalone RSI/MACD.
    long_score = (
    ...
    # Long-only mode uses slightly broader entry logic to recover opportunity.
    if enable_shorting:
        buy_mask = (
            macd_bull_cross
            & strong_up_trend
    ...
    # Exit triggers are easier than entries, with additional stateful risk controls below.
    exit_long_mask = macd_bear_cross & ((data['RSI'] > 60) | (data['Close'] < data['VWAP']) | (long_score <= 3))
    cover_mask = macd_bull_cross & ((data['RSI'] < 40) | (data['Close'] > data['VWAP']) | (short_score <= 4))

    # Stateful signal generation:
    # - avoid first/last minutes for fresh entries
    # - add cooldown after exits
    # - ATR-based stop-loss / take-profit to cap adverse moves and lock gains
    position = None
    entry_price = None
    entry_i = -1
    peak_price = None
    ...
    cooldown_bars = 0
    min_hold_bars = 2 if enable_shorting else 2
    post_exit_cooldown = 2 if enable_shorting else 2
    win_cooldown_bars = 1 if enable_shorting else 1
    ...
    for i in range(len(data)):
        ...
        # Side-specific time windows:
        # shorts are restricted to midday where historical performance was stronger.
    ...
    if not enable_shorting:
        # Keep mapping consistent when shorting is disabled.
...
def main():
    ...
    # Dynamically select stocks based on current market conditions
    ...
    for stock in stocks:
        try:
            ...
            data = data.dropna(subset=['RSI', 'MACD', 'MACD_signal', 'Close', 'EMA20', 'EMA50', 'VWAP', 'ADX'])
            if data.empty:
            ...
            print(f"{stock}: Signal - {latest_signal}, Confidence - {latest_confidence:.1f}%, Price - {latest_price}")
